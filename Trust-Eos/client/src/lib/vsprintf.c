...
uint32_t TEE_StrLen(char* p_str)
{
    uint32_t len = 0;
    if(!p_str){
        return 0;
...
unsigned int Client_Printf(const char *fmt, ...)
{ 
    va_list args;
    unsigned int i;
    char *p;
        ...
        if(*fmt == '%'){
            ...
            if(*fmt == '0'){
                ...
                if((zero_padding < 0x31) || (zero_padding > 0x38)){
                    pl011_uart_puts("invalid padding bits.\0");
                }
                zero_padding -= 0x30;
            }
            ...
            switch(*fmt){       
                ...
                default:
                {
                    pl011_uart_puts("option after %: ");
                    pl011_uart_putc(*fmt);
                    pl011_uart_puts("\r\n");
                    pl011_uart_puts("Unknown option after %\0");
                }
            } /* switch ends             */
        } /* if % character found      */
