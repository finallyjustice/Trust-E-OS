...
struct fs_list_head {
	struct fs_list_head *next, *prev;
};

#define FS_LIST_HEAD_INIT(name) { &(name), &(name) }

#define FS_LIST_HEAD(name) \
	struct fs_list_head name = FS_LIST_HEAD_INIT(name)

#define FS_INIT_LIST_HEAD(ptr) do { \
 ...
 */
static __inline__ void __fs_list_add(struct fs_list_head * new,
 ...
 */
static __inline__ void fs_list_add(struct fs_list_head *new, struct fs_list_head *head)
{
	__fs_list_add(new, head, head->next);
}
 ...
 */
static __inline__ void fs_list_add_tail(struct fs_list_head *new, struct fs_list_head *head)
{
	__fs_list_add(new, head->prev, head);
}
 ...
 * the prev/next entries already!
 */
static __inline__ void __fs_list_del(struct fs_list_head * prev,
				  struct fs_list_head * next)
{
	next->prev = prev;
	prev->next = next;
 ...
 */
static __inline__ void fs_list_del(struct fs_list_head *entry)
{
	__fs_list_del(entry->prev, entry->next);
}
 ...
 * @entry: the element to delete from the list.
 */
static __inline__ void fs_list_del_init(struct fs_list_head *entry)
{
	__fs_list_del(entry->prev, entry->next);
	FS_INIT_LIST_HEAD(entry);
}
 ...
 */
static __inline__ int fs_list_empty(struct fs_list_head *head)
{
 ...
 */
static __inline__ void fs_list_splice(struct fs_list_head *list, struct fs_list_head *head)
{
	struct fs_list_head *first = list->next;

	if (first != list) {
		struct fs_list_head *last = list->prev;
		struct fs_list_head *at = head->next;
